...
your actual aircraft. To set it up you will need an SSH client like: 
...
.. Tip::
  To copy paste into an SSH session use Ctrl-C to copy the text and use the right mouse button to insert into the SSH session on the command line.
...
For example, it should look like "output add 123.456.789.123:14551" but with your own IP address
...
Like on a real aircraft, the Throttle Safety Switch must be armed before the motor will engage for flight:
...
effects on the aircraft using these two commands and changing the numbers at the end (wind speed is in m/s and direction is in degrees from North)::

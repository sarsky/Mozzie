 ...
 * to simplify replacing the complete avionics if because of a malfunction
 ...
 * to allow for a quick replacement of the avionics hardware in the competition and testing and to reduce repair and maintenance time in the field
   (replacement being faster than repair)
...
As a consequence of the integration of the various components into the avionics package, including the pi Camera module and servo tilt mount,
it was possible to also use the mass of the enclosure to dampen the camera and FC from airframe and propulsion induced vibration.
It also considerably reduced the cabling complexity and cable use in the airframe which was a challenge for a small airframe, and also simplified the camera angle setup because the FC IMU was mounted on, and moved with, the same structure as the camera.
